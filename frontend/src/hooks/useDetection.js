...
// Lưu ý: Nếu bạn đã cấu hình Proxy trong vite.config.js thành '/api' như hướng dẫn trước,
// bạn có thể đổi dòng này thành: const API_BASE = '/api'
...
export function useDetection() {
    ...
    // TỐI ƯU HIỆU SUẤT: Tạo một canvas ẩn duy nhất để tái sử dụng
    ...
    const sendFrame = useCallback(async () => {
        ...
        // Sử dụng lại canvas ẩn thay vì tạo mới liên tục
    ...
    useEffect(() => () => {
        streamRef.current?.getTracks().forEach(t => t.stop())
        clearTimeout(intervalRef.current) // Đã sửa lỗi: dùng clearTimeout cho setTimeout
        clearInterval(fpsTimerRef.current)
    }, [])
